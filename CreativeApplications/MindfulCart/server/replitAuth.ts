...
export async function setupAuth(app: Express) {
  ...
  app.get("/api/login", (req, res, next) => {
    const hostname = req.get('host') || req.hostname; // Use full host including port for development
...
export const isAuthenticated: RequestHandler = async (req, res, next) => {
  ...
  if (process.env.NODE_ENV !== "production" && req.get('host')?.includes('localhost')) {
    // Create a mock user for development and ensure session is established
    (req as any).user = {
      claims: {
        sub: "dev-user-123",
    ...
    // Set up the session if it doesn't exist
